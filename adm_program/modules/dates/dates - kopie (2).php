...
$getView     = admFuncVariableIsValid($_GET, 'view',      'string', array('defaultValue' => $gSettingsManager->getString('dates_view'), 'validValues' => array('detail', 'compact', 'room', 'participants', 'description', 'songs')));
...
else
{
    ...
    if ($getView !== 'detail') // $getView = 'compact' or 'room' or 'participants' or 'description'
    {
        ...
        switch ($getView)
        {
            ...
            case 'participants':
                ...
                break;
            case 'songs':
                $columnHeading = array('&nbsp;', $gL10n->get('SON_OWNER'), $gL10n->get('SON_TITLE'), $gL10n->get('BND_NAME'));
                $columnAlign   = array('center', 'left', 'left', 'left');
                $compactTable->disableDatatablesColumnsSort(array(5));
                $compactTable->setColumnWidth(4, '35%');
                break;
    ...
    foreach($datesResult['recordset'] as $row)
    {
        ...
        if($getView === 'detail')
        {
            ...
            if ($date->getValue('dat_maxSlotSongs') != '')
            {
                $dateElements[] = array($gL10n->get('DAT_MAXSLOTSONGS'), '<strong>'.$date->getValue('dat_maxSlotSongs').'</strong>');
            }
            if ($date->getValue('dat_maxSlotDuration') != '')
            {
                $dateElements[] = array($gL10n->get('DAT_MAXSLOTDURATION'), '<strong>'.$date->getValue('dat_maxSlotDuration').'</strong> min');
            }
            ...
            {
                $page->addHtml('<h5>Description:</h5>
                        <p>' . $date->getValue('dat_description') . '</p>');
            }
            if($date->getValue('dat_concept') !='')
            {
                $page->addHtml('<h5>Concept:</h5>
                        <p>' . $date->getValue('dat_concept') . '</p>');
            }
            if($date->getValue('dat_ev_financial') !='' && ($gCurrentUser->isAdministrator() || $gCurrentUser->editDates()))
            {
                $page->addHtml('<h5>Financial agreements:</h5>
                        <p>' . $date->getValue('dat_ev_financial') . '</p>');
            }
            if($date->getValue('dat_ev_setupPlan') !='' )
            {
                $page->addHtml('<h5>Setup Plan:</h5>
                        <p>' . $date->getValue('dat_ev_setupPlan') . '</p>');
            }
            $page->addHtml($attentionDeadline);
