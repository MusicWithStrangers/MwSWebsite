    ...
    if($getId === 0)
    {
        ...
        else
        {
            $selectBoxEntries = array(
                ...
                'participants' => $gL10n->get('DAT_VIEW_MODE_COMPACT').' - '.$gL10n->get('SYS_PARTICIPANTS'),
                'description'  => $gL10n->get('DAT_VIEW_MODE_COMPACT').' - '.$gL10n->get('SYS_DESCRIPTION'),
                'songs'        => $gL10n->get('DAT_VIEW_MODE_COMPACT').' - '.$gL10n->get('SYS_SONGS')
            );
        }
        $form->addSelectBox(
...
else
{
    ...
    foreach($datesResult['recordset'] as $row)
    {
        ...
        if($getViewMode === 'html')
        {
            ...
            if ($date->isEditable())
            {
                $outputVenueContact = $date->getValue('dat_ev_venueContact');
                $outputFinancial = '<strong>Finance:</strong><br>'.$date->getValue('dat_ev_financial');
        ...
        $outputConcept = $date->getValue('dat_concept');
        $outputMaxSlotDuration = $date->getValue('dat_maxSlotDuration');
        $outputMaxSlotSongs = $date->getValue('dat_maxSlotSongs');
        ...
        if($date->getValue('dat_songregistration') )
        {
                                                    
            $buttonSongURL = safeUrl(ADMIDIO_URL.FOLDER_MODULES.'/songs/songs_show.php', array('mode' => 'html', 'dat_id' => $dateId));
            $outputButtonSongRegister  = '
                        <button class="btn btn-default" onclick="window.location.href=\'' . $buttonSongURL . '\'"><img src="' . THEME_URL . '/icons/songs.png" alt="Register Songs" />' . 'Register Songs' . '</button>';
        }
        ...
        if($getView === 'detail')
        {
            ...
            if($outputMaxSlotDuration !== '')
            {
                $dateElements[] = array($gL10n->get('DAT_MAXSLOTDURATION'), '<strong>' . $outputMaxSlotDuration . '</strong>');
            }
            if($outputMaxSlotSongs !== '')
            {
                $dateElements[] = array($gL10n->get('DAT_MAXSLOTSONGS'), '<strong>' . $outputMaxSlotSongs . '</strong>');
            }
            if($outputVenueContact !== '')
            {
                $dateElements[] = array($gL10n->get('DAT_VENUECONTACT'), '<strong>' . $outputVenueContact . '</strong>');
            }
            ...
            || $outputButtonParticipantsEmail !== '' || $outputButtonParticipantsAssign !== '' || $outputButtonSongRegister !== '')
            {
                $page->addHtml('<div class="btn-group">' . $outputButtonParticipation . $outputButtonParticipants . $outputButtonParticipantsEmail . $outputButtonParticipantsAssign . $outputButtonSongRegister . '</div>');
            } elseif ($outputButtonSongRegister !== '')
            {
                    $page->addHTML('<div class="btn-group">' . $outputButtonSongRegister) . '</div>';
            }
            $page->addHtml('
                </div>
