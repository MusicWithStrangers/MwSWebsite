...
$dateSongRegistrationPossible = false;
...
else
{
    ...
    if($date->getValue('dat_songregistration') > 0)
    {
        $dateSongRegistrationPossible = true;
    }
    // check if current user is assigned to this date
...
$page->addJavascript('
    ...
    function setDateSongRegistration() {
        if ($("#dat_songregistration:checked").val() !== undefined) {
            $("#dat_maxGuests_group").show("slow");
            $("#dat_maxBands_group").show("slow");
            $("#dat_maxSongParticipate_group").show("slow");
            $("#dat_changeOverTime_group").show("slow");
            $("#dat_maxSlotDuration_group").show("slow");
            $("#dat_maxSlotSongs_group").show("slow");
            $("#dat_maxBandsTotalEvent_group").show("slow");
            $("#dat_maxDurationTotalEvent_group").show("slow");
        } else {
            $("#dat_maxGuests_group").hide();
            $("#dat_maxBands_group").hide();
            $("#dat_maxSongParticipate").hide();
            $("#dat_changeOverTime_group").hide();
            $("#dat_maxSlotDuration_group").hide();
            $("#dat_maxSlotSongs_group").hide();
            $("#dat_maxBandsTotalEvent_group").hide();
            $("#dat_maxDurationTotalEvent_group").hide();
        }
    }
    /**
...
$page->addJavascript('
    ...
    setDateSongRegistration()
    setLocationCountry();

    $("#dat_songregistration").click(function() {
        setDateSongRegistration();
    });
...
$form->addCheckbox(
    'dat_songregistration', $gL10n->get('DAT_SONGREGISTER'), (bool) $date->getValue('dat_songregistration'),
    array('helpTextIdLabel' => 'DAT_SONGLOGIN_POSSIBLE')
);
...
//$form->addCheckbox('dat_songregistration', $gL10n->get('DAT_SONGREGISTER'), (bool) $date->getValue('dat_songregistration'));
...
$form->addInput('dat_maxSlotDuration', 'Max Slot duration', $date->getValue('dat_maxSlotDuration'),
        array('type' => 'number', 'minNumber' => 0, 'maxNumber' => 60, 'step' => 1));
$form->addInput('dat_maxSlotSongs', 'Max songs', $date->getValue('dat_maxSlotSongs'),
        array('type' => 'number', 'minNumber' => 0, 'maxNumber' => 40, 'step' => 1));
$form->addInput('dat_changeOverTime', 'Change over time', $date->getValue('dat_changeOverTime'),
        array('type' => 'number', 'minNumber' => 0, 'maxNumber' => 60, 'step' => 1));
$form->addInput('dat_maxBandsTotalEvent', 'Max bands total event', $date->getValue('dat_maxBandsTotalEvent'),
        array('type' => 'number', 'minNumber' => 0, 'maxNumber' => 60, 'step' => 1));
$form->addInput('dat_maxDurationTotalEvent', 'Max duration total event', $date->getValue('dat_maxDurationTotalEvent'),
        array('type' => 'number', 'minNumber' => 0, 'maxNumber' => 500, 'step' => 1));
$form->addInput(
    'dat_maxSongsParticipate', $gL10n->get('DAT_MAXTOTALSONGS'), $date->getValue('dat_maxSongParticipate'),
    array('type' => 'number', 'minNumber' => 0, 'maxNumber' => 20, 'step' => 1, 'helpTextIdLabel' => 'DAT_MAX_TOTALSONGS')
);
$form->addInput(
    'dat_maxGuests', $gL10n->get('DAT_MAXGUESTS'), $date->getValue('dat_maxGuests'),
    array('type' => 'number', 'minNumber' => 0, 'maxNumber' => 5, 'step' => 1, 'helpTextIdLabel' => 'DAT_MAX_GUESTS')
);
$form->addInput(
    'dat_maxBands', $gL10n->get('DAT_MAXBANDS'), $date->getValue('dat_maxBands'),
    array('type' => 'number', 'minNumber' => 0, 'maxNumber' => 5, 'step' => 1)
);
...
$form->addEditor('dat_ev_financial', '', $date->getValue('dat_ev_financial'));
$form->closeGroupBox();

$form->openGroupBox('gb_setup', $gL10n->get('DAT_SETUPPLAN'), 'admidio-panel-editor');
$form->addEditor('dat_ev_setupPlan', '', $date->getValue('dat_ev_setupPlan'));
$form->closeGroupBox();
