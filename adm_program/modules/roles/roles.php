...
while($row = $rolStatement->fetch())
{
    ...
    if($role->getValue('rol_boolingadmin') == 1)
    {
        $assignRoles .= '<img class="admidio-icon-info" src="'. THEME_URL. '/icons/bookings.png"
                            alt="Edit bookings" title="Edit bookings" />';
    }
    if($role->getValue('rol_bookandregister') == 1)
    {
        $assignRoles .= '<img class="admidio-icon-info" src="'. THEME_URL. '/icons/songs.png"
                            alt="Book rooms and register to events" title="Book rooms and register to events" />';
    }
