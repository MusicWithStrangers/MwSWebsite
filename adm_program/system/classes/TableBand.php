 ...
 * ```
 */
class TableBand extends TableAccess
{
    /**
     ...
     */
    public function __construct(Database $database, $bndId = 0)
    {
        // read also data of assigned category
        //$this->connectAdditionalTable(TBL_CATEGORIES, 'cat_id', 'dat_cat_id');

        parent::__construct($database, TBL_BANDS, 'bnd', $bndId);
    }
    ...
    {
        $bndId     = (int) $this->getValue('bnd_id');
    ...
    public function getValue($columnName, $format = '')
    {
        ...
        if ($columnName === 'bnd_description')
        {
            if (!isset($this->dbColumns['bnd_description']))
            {
                $value = '';
            }
            elseif ($format === 'database')
            {
                $value = html_entity_decode(strStripTags($this->dbColumns['bnd_description']), ENT_QUOTES, 'UTF-8');
            }
            else
            {
                $value = $this->dbColumns['bnd_description'];
            }
        }
        else
        {
    ...
    public function isEditable()
    {
        ...
        if ($gCurrentUser->getValue('usr_id' === $this->getValue('bnd_usr_id')))
        {
